...
class MemoryBucket {
  ...
  isAllowed(key){
    ...
    if (!this.cooldown[key] && size <= 0 && size + tokens < this.limit){
      ...
      this.logger.info(`${showts()} ${key} cooldown started`)
    }
    else if (this.cooldown[key] && size + tokens >= this.limit){
      this.logger.info(`${showts()} ${key} cooldown lifted after ${Date.now() - this.cooldown[key]}ms`)
      delete this.cooldown[key]
    }
    ...
    if (this.verbose){
      ...
      this.logger.debug(`${showts()} ${key} bkt: ${size.toFixed(3)}  incr ${tokens.toFixed(3)}${old_tokens}  ${allow}`)
    }
