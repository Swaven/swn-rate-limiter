  ...
  isAllowed(key){
    ...
    if (!this.buckets[key] || this._isOverTTL(this.buckets[key].ts)){
      ...
      if (this.cooldown[key]){
        this.logger.info(`${utils.showts()} ${this.name}:${key} cooldown lifted after ${Date.now() - this.cooldown[key]}ms`)
        delete this.cooldown[key]
        utils.sns(this.sns, `${this.name}:${key}`, false, this.appName, this.rate)
      }

      if (this.verbose)
        this.logger.debug(`${utils.showts()} ${this.name}:${key} new bucket`)
    ...
    if (!this.cooldown[key] && size <= 0 && size + tokens < this.limit){
      ...
      this.logger.info(`${utils.showts()} ${this.name}:${key} cooldown started`)
      utils.sns(this.sns, `${this.name}:${key}`, true, this.appName, this.rate)
    }
    ...
    if (this.verbose){
      ...
      this.logger.debug(`${utils.showts()} ${this.name}:${key} bkt: ${size.toFixed(3)}  incr ${tokens.toFixed(3)}${old_tokens}  ${allow}`)
    }
