...
const redis = require('redis'),
      ...
      RedisBucket = require('./redisBucket.js'),
      MemoryBucket = require('./memoryBucket.js'),
...
function validateOptions(opts){
  ...
  opts.name = opts.name || randomString(5)
...
function createLimit(opts){
  ...
  if (opts.local)
    limiters[name] = new MemoryBucket(opts, defaults.sns)
  else{
    if (!client)
      throw new Error('Shared bucket needs Redis connection')
    limiters[name] = new RedisBucket(opts, client, defaults.sns)
  }

  opts.logger.info(`new ${opts.local ? 'local': 'shared'} limit: ${opts.rate.expr}`)
...
function setup(opts){
  ...
  defaults.appName = opts.appName || ''

  if (opts.sns){
    if (!opts.sns.service || !opts.sns.arn)
      throw new Error('sns config must include service object and arn')
    defaults.sns = opts.sns
  }

  if (opts.redis){
    client = redis.createClient(opts.redis, {
      prefix: 'swn-rl:' + defaults.appName + ':',
    ...
    client.on('ready', () => {
      defaults.logger.info('client connected & ready')
    })
